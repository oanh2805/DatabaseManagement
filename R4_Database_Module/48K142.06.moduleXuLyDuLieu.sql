...
EXEC ThayDoiMatKhau @TenDangNhap = '220000236', @MatKhauCu = 'password236', @MatKhauMoi = 'newpassword123';
...
--Câu 5: Kiểm tra nhân viên có phần trăm lỗi > 20% khí biết mã nhân viên. Nếu có thì in ra tên nhân viên 
--và đưa ra thông báo 'Cảnh báo đuổi việc'. Ngược lại thì đưa ra thông báo 'Không cảnh báo đuổi việc'.
create or alter proc pLoik  ( @manv varchar(15), @ten nvarchar (50) output, @nx nvarchar (50) output )
as 
begin
	declare @phat int
...
BEGIN
    ...
    END
END
exec sp_ThemNhanVien '0001', N'Lê Hữu Đức Trọng', '2003-11-07', N'Nữ', N'Pleiku', '0982231421', 'N0621'
select *from nhanvien
...
AS
BEGIN
    -- Quy định giờ vào và giờ ra cho hai ca làm việc (ca 1: ca chiều; ca 2: ca sáng)
